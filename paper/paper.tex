...
concepts \citep{cameron2008bootstrap}, since the 40 points are four
...
\begin{figure}[tb]
  ...
  \texttt{topic\_science} on each of the four models, which carries the highest
...
steerable. Figure~\ref{fig:inversion} plots the relationship.
...
control) as a secondary axis with more spread; Appendix~\ref{app:selectivity}
shows both axes side by side and explains why selectivity is reported
alongside the preregistered axis rather than substituted for it.
...
model produced the text or at what steering strength, so the labels cannot
have been shaped by knowing whether an item came from a steered or unsteered
...
\textbf{Ten clusters is few.} The cluster bootstrap resamples concepts rather
than points, because the 40 observations are four measurements of each of ten
concepts. Ten is a small number of clusters, and inference with few clusters is
known to over-reject relative to its nominal size \citep{cameron2008bootstrap}.
The interval on the primary test should therefore be read as indicative rather
than exact, and the direction of the estimate carries more weight here than its
precise endpoints. Adding concepts, not adding models, is what would tighten
this.
...
Readability is the preregistered $x$-axis and it is saturated: 31 of 40 points
...
\begin{figure}[htbp]
  ...
  \caption{The same 40 points against two candidate readability axes.
...
Seventeen of the forty points entered it; the rest were already moved by the
default intervention and never reached it.

The table is the evidence behind the terminology distinction. Nine points
resist all six interventions and are called immovable. Eight are moved by at
least one, and the column showing which one is what separates those two claims:
\texttt{formality} on Qwen and Llama and \texttt{verbosity} on Qwen and Gemma
each yield to a specific intervention while resisting the default one, which is
exactly the case that a single-intervention protocol would have misreported as
immovable. \texttt{topic\_science} resists all six on all four models, and
\texttt{refusal} resists all six on two of four.

\begin{table}[htbp]
  \caption{Every concept-model point that entered the robustness battery, with
  ...
  \begin{tabular}{llccccccc}
    ...
    \texttt{formality} & Gemma-2-9b & 0.012 & 0.048 & 0.030 & 0.000 & 0.006 & 0.011 & immovable \\
    ...
    \texttt{refusal} & Gemma-2-9b & 0.018 & 0.041 & 0.089 & 0.080 & 0.018 & 0.028 & moved \\
    ...
    \texttt{sycophancy} & Gemma-2-9b & 0.006 & 0.016 & 0.009 & 0.002 & 0.006 & 0.012 & immovable \\
    \texttt{topic\_science} & Gemma-2-9b & 0.005 & 0.005 & 0.007 & 0.006 & 0.003 & 0.006 & immovable \\
    ...
    \texttt{verbosity} & Gemma-2-9b & 0.028 & 0.058 & 0.011 & 0.012 & 0.022 & 0.032 & moved \\
