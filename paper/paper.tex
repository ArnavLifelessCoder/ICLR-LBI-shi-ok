...
The validation is the contribution. We build four concepts whose behavioral
readout is a rule over the generated string rather than a judgment, so no model
scores the outcome and the intended direction follows from construction: word
count, the fraction of letters capitalized, the fraction of characters that are
digits, and the fraction of words that are French function words. Across four
instruction-tuned models, 5 of 16 points have a signed area whose interval
excludes zero, and \emph{every one is positive}. Nothing anywhere in the study
resolves against the direction it was built from. Steering into French resolves
on three of four models and into capitals on two. The check therefore detects
real steering when real steering is present, which is what licenses reading its
nulls as statements about concepts rather than about the instrument.

With that established, the same measurement on ten judge-scored concepts
behaves differently, and the difference is the judge. Where a rule computes the
readout, the absolute and signed summaries almost coincide, with a median
$0.993$ of the measured effect attributable to directional movement. Where an
LLM judge computes it, the median is $0.387$ and the two summaries are
uncorrelated. Re-scoring identical sweeps with a second judge moves absolute
controllability by between $0.5\times$ and $26.8\times$ and flips 3 of 16 signs,
so a published controllability number is not comparable across judges. A
danger-zone criterion from our own preregistration selects one concept under the
smaller judge and none under the larger.
...
That licenses the second claim. Applying both summaries to ten judge-scored
...
effect that is directional is $0.993$; with an LLM judge it is $0.387$, and the
two summaries are uncorrelated. Re-scoring identical sweeps under a second judge
...
$0.215$ Qwen, $0.163$ Mistral, $0.101$ Llama, $0.110$ Gemma; floor $0.10$). Read
directionally the control is passed by two. Signed sentiment area resolves
positive on Qwen ($+0.21$) and Mistral ($+0.16$), and does not resolve on Llama
($+0.020$, CI $[-0.016, 0.066]$) or Gemma ($-0.039$, CI $[-0.107, 0.002]$). We
withhold Gemma, whose estimate is negative, and retain Llama, whose estimate is
positive.

That is a judgment call on unresolved signs and we would rather state it than
let it pass as a rule. A strict directional control, demanding that the control
\emph{demonstrate} steering rather than merely fail to contradict it, withholds
Llama as well and leaves two models. Table~\ref{tab:withhold} reports the
...
Figure~\ref{fig:groundtruth} and Table~\ref{tab:groundtruth} give the
sixteen ground-truth points, four concepts on four models, swept on a common
grid with no judge anywhere in the measurement; Appendix~\ref{app:groundtruth}
gives every point with its interval and arm split.

Five of the sixteen have a signed area whose interval excludes zero, and
\emph{every one of the five is positive}. Nothing anywhere in this study,
on these concepts or the judge-scored ones, resolves against the direction it
was estimated from. Steering into French resolves on three of the four models
($+0.057$, $+0.054$, $+0.040$) and steering into capitals on two ($+0.181$,
$+0.073$). Where the sign resolves, it is the sign construction predicts.

\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth]{fig9_groundtruth.png}
  \caption{(a) Directional controllability on the sixteen ground-truth
  points, where the readout is a rule and the intended direction is known by
  construction. Resolved points are bold; all five are positive. (b) The share
  of measured effect attributable to directional movement, for points with a
  non-trivial effect. Where a rule computes the readout the two summaries
  almost coincide; where an LLM judge computes it they do not.}
  \label{fig:groundtruth}
\end{figure}

Two concepts do not resolve on any model and neither reason is mysterious.
\texttt{gt\_digits} moves only past the fluency ceiling: on Qwen it reaches
$0.21$ at $\alpha=+4$ and $0.55$ at $+5$, both excluded on perplexity, so the
usable range ends before the behavior starts. \texttt{gt\_length} has no
consistent response anywhere, and it is the one concept whose unsteered
baseline sits mid-range rather than at a floor; Section~\ref{sec:limitations}
returns to that.

The importance of this section is what it licenses rather than what it shows.
A directional summary that never resolved anything would be indistinguishable
from a broken one, and every null elsewhere in the paper would be
uninterpretable. Because the check resolves the correct sign where the correct
sign is knowable, a null on a judge-scored concept is a statement about that
concept or that judge, not about the metric.
...
non-trivial effect it is $0.993$ (Figure~\ref{fig:groundtruth}b). Removing
...
\textbf{The signed summary rarely resolves a judge-scored direction.}
Propagating each coefficient's prompt-level bootstrap interval into the signed
area (Appendix~\ref{app:signed}) leaves 3 of the 30 retained points with a
sign distinguishable from zero, all positive. Sixteen points have a negative
signed area and \emph{none} is resolved
(Appendix~\ref{app:absvsdir} breaks this down by concept), which is why we make no claim about
wrong-way movement: sixteen wrong-way point estimates are equally what an
uninformative judge produces. The supportable claim is conditional. With a
judge validated on one concept and six evaluation prompts, direction is
unresolvable on 27 of 30 points. Section~\ref{sec:groundtruth} is what keeps
that from being a statement about the metric.

\textbf{The effect is present where the judge is validated.} The three resolved
points are not distributed at random across concepts. Two of them are
...
cannot validate itself. We validate it on four concepts whose readout is a rule
over the generated string, where the correct answer follows from construction:
across four models, 5 of 16 points resolve and every one is positive, with
steering into French resolving on three models and into capitals on two. No
point anywhere in the study resolves against the direction it was built from.
...
summaries almost coincide, median share $0.993$; where an LLM judge computes it
the median is $0.387$ and the two are uncorrelated. Re-scoring identical sweeps
under a second judge moves magnitudes by up to $26.8\times$ and flips 3 of 16
signs, and a preregistered criterion of ours selects one concept under one judge
and none under the other. The divergence the paper began by documenting belongs
to the judge, not to steering.
...
negative entry would be evidence the check is wrong. There are none.
...
\begin{table}[htbp]
  ...
  direction is therefore known in advance. 5 of 16 points have an interval
