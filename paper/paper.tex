...
steering a model in the intended direction from perturbing it, and that the data
it produces cannot by itself settle which of the two occurred. For ten concepts
across three instruction-tuned models we sweep a steering coefficient to a
preregistered fluency ceiling and compute two summaries of the same curves: the
usual area under the absolute behavioral deviation, and its signed analogue,
...
$-0.01$ over 30 concept-model points) and disagree materially on 27 of 30, with
the median point attributing $39$\% of its measured effect to directional
movement.

The signed summary is not offered as the correct answer, because it usually has
no answer. Propagating each coefficient's prompt-level interval into the signed
area leaves 3 of 30 points with a sign distinguishable from zero. All three are
positive, and two are the concept on which our judge is validated against a
human. We therefore report the conditional claim the evidence supports rather
than the quotable one: with a judge validated on a single concept and six
evaluation prompts, the direction of a steering effect is unresolvable on 27 of
30 points, and no directional conclusion follows from either summary. Splitting
the signed area by arm shows the problem is not only statistical power, since 24
of 30 points respond on one side of the baseline only, the positive control
among them.

We then show what this costs a substantive result. We preregistered the
hypothesis that a direction is steerable to the extent it projects into the
model's output-effective (unembedding) subspace. Under the absolute metric the
estimate is inverted (partial Spearman $-0.45$, conditioning on readability),
and it survives the checks ordinarily asked of such a result: output overlap is
unrelated to where the fluency ceiling falls, 27 of 30 sweeps reach the end of
the coefficient grid unbroken, and leave-one-concept-out keeps the estimate
negative in every refit. Under the signed metric the same test returns $+0.14$.
A result can pass the diagnostics the field asks for and still be a property of
its own summary statistic. Held-out probe AUROC is $1.00$ at 24 of 30 points, so
the near-zero readability-controllability correlation is uninformative rather
than a null, and the supportable claim is that high linear readability does not
guarantee an effective control handle. A fourth model is withheld throughout for
failing the preregistered positive control under the directional reading.
\end{abstract}
...
controllability of Section~\ref{sec:method}. The two summaries of the same
curves are uncorrelated across the 30 retained points (Spearman $-0.01$) and
disagree materially on 27 of 30: the median point attributes $39\%$ of its
measured effect to directional movement, and no point reaches the line on which
the two agree (Figure~\ref{fig:directional}). Whatever the absolute metric is
measuring, it is largely not direction.
...
\begin{figure}[tb]
  ...
  of the same curves are unrelated. Points below the line have a negative signed
  area, though none of those signs is resolved once prompt-level uncertainty is
  propagated (Appendix~\ref{app:signed}). (b) The share of each concept's
  measured effect attributable to directional movement, median over models;
  nothing reaches $1.0$.}
  \label{fig:directional}
\end{figure}

\textbf{The signed summary rarely resolves a direction.} Per-prompt scores are
not retained, but every coefficient carries a prompt-level bootstrap interval,
and propagating those into the signed area by Monte Carlo bounds what the sign
can support (Appendix~\ref{app:signed}). Three of the 30 points have a signed
area distinguishable from zero. All three are positive: \texttt{sentiment} on
Qwen ($+0.21$) and Mistral ($+0.16$), and \texttt{rudeness} on Qwen ($+0.12$).
Sixteen points have a negative signed area and \emph{none} of them is resolved.

This is the reason we do not claim that steering moves models in random
directions, and it is worth being explicit that an earlier version of this
analysis did. Sixteen wrong-way points, a near-zero median monotonicity, and
cross-model sign agreement at chance are equally what a uniformly uninformative
judge would produce. The data cannot separate the two, so the supportable claim
is the conditional one: with a judge validated on a single concept and six
evaluation prompts, the direction of a steering effect is unresolvable on 27 of
30 points, and no directional conclusion follows from either summary.
...
\textbf{A sign-convention error would look like this too, and does not explain
it.} \texttt{certainty} has a negative response on every model, which is what an
inverted direction would produce. Its stimuli contrast ``It is beyond doubt
that'' with ``It is hardly settled that'', correctly signed, and a systematic
polarity error would show many concepts agreeing across models rather than the
two of ten that chance predicts. With intervals attached the question dissolves:
\texttt{certainty} is unresolved on all three models, so there is no effect to
attribute to a bug.
...
A second implication concerns what probes license. Among concepts this probe
recovers essentially perfectly, controllability varies by a factor of twenty, so
a working probe is not evidence of a control handle. The converse, that
unreadable concepts are uncontrollable, is not something this design can address.
We state the asymmetry because the more quotable version, that detection does not
predict control, would require variance on an axis that has none here.

Neither implication says concept directions never control behavior; the existence
proofs in Section~\ref{sec:related} stand. What this data questions is the
inference from a measured effect size to a claim of control, when the measurement
...
\textbf{The judge is the limiting factor, not one caveat among several.}
Controllability is a function of the judge end to end, so an unresolved
directional effect and an uninformative judge predict the same scores, and
nothing in this data separates them. The one place they come apart is
\texttt{sentiment}, where the judge is validated and where steering is
directional on all three models. Every directional statement in this paper is
conditional on that, which is why the abstract reports what is unresolvable
rather than what moved. A judge of comparable scale to the models being judged,
and validation on more than one concept, are what would lift the condition.
...
concepts that score is uncorrelated with whether the model moved in the intended
direction, and the two disagree on 27 of 30 points. What we cannot do is say
which direction the model moved: once prompt-level uncertainty is propagated,
the sign resolves on 3 of 30 points, all positive, two of them on the one
concept where our judge is validated. A preregistered geometric hypothesis
tested against the usual score comes out inverted, survives a fluency-ceiling
control and leave-one-concept-out, and then disappears when the score is made
directional, which is the concrete cost of the ambiguity. Among concepts our
probe recovers at AUROC $1.0$ controllability still varies twentyfold, so a
working probe is not evidence of a control handle; the converse is not something
this design can test. The recommendation is small and cheap: report a signed
dose-response next to any absolute one, with intervals and split by arm, and
treat their agreement as a condition for interpreting the magnitude. The
measurement we most need and do not have is a judge validated per concept
rather than on the positive control alone.
