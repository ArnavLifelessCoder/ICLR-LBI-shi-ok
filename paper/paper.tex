...
activations, added to the residual stream at a range of strengths, and the
behavioral shift is summarized as a magnitude: how far the model moved. That
summary is silent on a question it appears to answer, whether the model moved the
way the direction was supposed to move it. An intervention that displaces
activations along an arbitrary direction will change what a model says, and a
metric recording the size of that change will score it. Only a metric that tracks
the sign can separate control from disturbance. We compute both from the same
dose-response curves and find them unrelated: over 30 concept-model points the
absolute and signed summaries correlate at $-0.01$, and the median point owes
under $40$\% of its measured effect to movement in the intended direction.
...
the subspace driving the model's output distribution. Measured with the absolute
metric the relationship is inverted, and the inversion survives the checks that
would ordinarily be asked of it: it is not produced by the fluency ceiling
truncating the sweep, and it does not depend on any single concept. Measured with
the signed metric it disappears. The checks are what make the point: a result can
pass the diagnostics the field asks for and still be an artifact of the summary
...
movement along this direction? To first order the behavior change is
$\alpha \lVert \nabla f(h) \rVert \cos(\hat v, \nabla f(h))$, for $\hat v$ the
unit concept direction and $f$ the map from the residual stream to the behavior
score, so controllability depends on how much of $\hat v$ lies in the subspace
the downstream computation is sensitive to. Readability instead follows a
Fisher-style ratio of between-class to within-class variance along $\hat v$. A
concept can separate its classes cleanly along a direction later layers barely
read.
