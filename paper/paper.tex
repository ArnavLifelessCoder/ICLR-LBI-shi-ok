...
computes it, the median is $0.558$. Measured on the same ten-concept count,
four models, grid, layer, thirty prompts and interval, so that the readout is
the only thing that differs, the rule resolves 26 of 40 signs and the judge 14
of 40 (Fisher $p=0.013$). Four of the judge's fourteen are \texttt{sentiment},
the one concept on which the judge is validated against human raters; without
it the judge resolves 10 of 36. Enlarging the judge's evaluation set fivefold
does not rescue it: the intervals tighten as sampling predicts, but the point
estimates collapse toward zero and 7 of 16 signs flip, so the small-sample
estimates were not merely uncertain. Re-scoring identical sweeps
...
We then apply the check to a published steering result. On the canonical
activation-addition demonstration, the wedding vector on GPT-2-XL at its own
layer and coefficient, wedding vocabulary rises monotonically with the
coefficient (Spearman $+0.96$) and the signed area resolves positive
($+0.0023$, CI $[+0.0016, +0.0031]$) with a directional share of $1.000$. The
published effect survives the check, which we preregistered as one of two
reportable outcomes. A diagnostic that only ever subtracts from other people's
results is a complaint about measurement; one that confirms an effect where the
effect is real and declines to confirm a judge-scored one where it is not is
doing the work a diagnostic is for.
...
effect that is directional is $1.000$; with an LLM judge it is $0.558$, and
the two summaries are only weakly related (Spearman $+0.29$ against $+0.80$
under the rule). On a fully matched protocol, forty points a side, the rule
resolves 26 signs and the judge 14. Re-scoring identical sweeps under a second judge
...
differently. Over the forty matched-protocol points the median share of
measured effect attributable to directional movement is $0.558$ (Mann--Whitney
against the rule readout, $p=0.001$), and the two summaries are only weakly
related (Spearman $+0.29$, $p=0.07$, against $+0.80$ under the rule). Over the twelve ground-truth points with a
...
We measured all ten judge-scored concepts exactly as the ten rule-scored ones
are measured: the same four models, a single layer, the thirteen-point grid,
thirty evaluation prompts and an interval for the mean, so the readout is the
only thing that varies. The rule resolves 26 of 40 signs and the judge 14 of 40,
$65\%$ against $35\%$ (Fisher exact $p=0.013$, odds ratio $3.45$). Of the
judge's fourteen, twelve are positive and two resolve the wrong way, both on
Mistral (\texttt{refusal} $-0.058$, \texttt{verbosity} $-0.107$), against one
wrong-way point in twenty-six under the rule. Appendix~\ref{app:signed} gives
the per-point intervals and arm splits.

Where the judge's resolutions fall is itself informative. \texttt{sentiment}
resolves on all four models, and it is the one concept on which the judge is
validated against human raters. \texttt{honesty} and \texttt{sycophancy}
resolve nowhere. Without \texttt{sentiment} the judge resolves 10 of 36,
$28\%$ (Fisher $p=0.001$ against the rule). The judge recovers direction
roughly where it has been shown to measure the concept, and mostly not
elsewhere.
...
ground-truth set's thirty. Beyond matching the prompt count as above, we
repeated the four load-bearing concepts at six and at thirty prompts with the
judge, grid and models held fixed, which isolates what the larger sample
does. The intervals tighten roughly
...
Everything above measures concepts we built. The obvious question is whether
the check changes anything about a result somebody else has already published,
and we preregistered both answers before running it (the commitment is in the
released code, in \texttt{lbi/published.py}).

We replicate the canonical activation-addition demonstration
\citep{turner2023actadd}: the wedding topic vector on GPT-2-XL, with
$p^{+}$ the single word \texttt{weddings}, $p^{-}$ a single space, layer $16$, coefficient $1$,
which is the running example of that paper's Section 4.1. The steering vector
is the position-wise difference between the two contrast prompts after
right-padding them to a common token length, added at the front of the user
prompt, exactly as their Algorithm 1 specifies. The readout is a word list over
the generated string, so no judge is involved and the outcome is decidable. We
sweep multiples of their coefficient, so $1.0$ is their setting itself rather
than a conversion between our units and theirs, and we sample rather than
decode greedily, because their demonstration is a sampling result.

The effect replicates and it is directional. Wedding vocabulary rises
monotonically with the coefficient (Spearman $+0.96$), from $0.00073$ at
baseline to $0.00148$ at their setting and $0.0116$ at four times it, with no
point breaking the fluency ceiling anywhere in the swept range. The signed area
is $+0.00233$ with interval $[+0.00156, +0.00312]$, so the sign resolves
positive, and the directional share is $1.000$: every unit of measured effect
is movement in the intended direction. Under the preregistration this is the
branch in which the published effect survives the check.

We think that is the more useful outcome. A diagnostic that only ever subtracts
from other people's results is easy to dismiss as a complaint about
measurement; one that confirms a published effect where the effect is real, and
declines to confirm a judge-scored one where it is not, is doing the work a
diagnostic is for. Activation addition, measured on a rule rather than a judge,
is one of the cases where absolute and directional controllability agree
completely.

\textbf{What it cost to get there, and why we report that.} Three successive
versions of this replication returned a readout that was flat at zero, and each
time the cause was ours rather than the method's. The first decoded greedily
against a sampling result, so a coefficient-scale nudge could not move an argmax
path. The second swept coefficients in residual-RMS units against a raw
activation difference whose norm was roughly $175$, so the grid was never shown
to contain their operating point. The third used a pooled last-token vector in
place of the position-wise difference. Only after all three were fixed did it
become clear that the remaining fault was not in the code at all: we had
recorded the intervention layer as $6$ where the paper says $16$, early in the
stack rather than the middle, where that paper reports intervening is most
effective.

Each of those nulls would have been publishable-looking and wrong. What
prevented it was a preregistered positive control requiring the sweep to
reproduce the effect somewhere in its range before any null could be reported.
We recommend the practice generally: a replication that cannot demonstrate the
effect it is re-scoring has not earned the right to re-score it.
...
the relationship disappears. On the thirty points this test is computed over,
the absolute metric is uncorrelated with the directional one (Spearman
$-0.01$, Table~\ref{tab:directional}), so the honest reading is
...
\textbf{The headline comparison is matched; the main study's older runs are
not.} The rule-versus-judge comparison is forty points a side, measured under
one protocol with per-generation scores retained, so its intervals are
recomputable from the released data. The paper's earlier judge-scored runs,
which feed the geometric test and the danger zone, predate the interval
correction and did not retain per-generation scores. Every quantity from those
runs that does not depend on a per-coefficient interval is unaffected and is
reported: the signed areas, directional shares, arm splits, judge-swap
magnitudes and sign flips, and the geometric test, whose controllability
intervals come from a prompt-level bootstrap that was always an interval for the
mean. No resolution count from those runs is reported.

\textbf{One judge, validated on one concept.} The judge resolves direction
four times in four on \texttt{sentiment}, where it agrees with human raters, and
ten times in thirty-six elsewhere. That pattern is consistent with the judge
being the limiting factor, and it is equally consistent with sentiment simply
being easier to steer. Separating the two needs judges validated on more
concepts, which we have not done.
...
the median is $0.558$ and the two are only weakly related. On a fully matched
protocol, forty points a side, the rule readout resolves $65\%$ of its
points and the judge $35\%$, and $28\%$ once its one validated concept is set
aside; enlarging the judge's evaluation set fivefold tightens its
...
Applied to a published steering result the check confirms rather than
subtracts: the activation-addition wedding effect on GPT-2-XL is monotone in
the coefficient and resolves positive with a directional share of $1.000$. That
is the outcome we preregistered as the one in which the published effect
survives, and it is the reason we think the check is usable rather than merely
critical.
