...
Interpretability-based safety quietly assumes that if you can read a concept off a model's activations, you have a handle on it: probe for deception, probe for harmful intent, and you are part way to controlling it. This paper shows that assumption is false in a measurable way, and that the obvious geometric explanation for why is not merely incomplete but pointed backwards. Across four model families and ten concepts we measure two things separately: how well a linear probe *detects* a concept (readability) and how much steering along that concept's direction actually *changes behavior* (controllability). The two come apart: a concept can be perfectly legible yet nearly immovable, resisting every standard steering intervention while an identical pipeline moves a control concept cleanly. We then test the natural first-order account of the gap, that a concept is steerable to the extent its direction lies in the model's output-effective subspace, and find it inverted: the concepts most aligned with that subspace are the *least* steerable, not the most, and this holds across all four models. We give the mechanistic reading this points to, that the feature is tracking how lexical a concept is rather than how much behavior depends on it, and we show that safety-relevant concepts can land on the immovable side of the gap, which means a good probe can give false confidence about control. The honest arc is therefore not "we can predict the gap from geometry" but "the obvious geometric predictor points the wrong way, and here is why," which is a stronger claim about the field's working picture of steering than a clean predictor would have been.
...
**What this buys.** Three things. It gives Experiment 4 a preregistered primary hypothesis instead of a regression over whatever features were available. It gives the danger zone a mechanistic story rather than a scatter plot. And it makes a null result informative: if none of M1, M2 or M3 predicts the gap, the honest conclusion is that the gap is not explained by the standard first-order picture of steering, which is a real statement about a real gap in the field's understanding rather than a shrug. As it turned out, the data did something stronger than the anticipated null (below).

**What the 40-point data did to M1 (added 2026-09-04, after nb7).** The prediction stated above is left exactly as preregistered; this paragraph is the observed outcome, not a revised hypothesis. Across four models and ten concepts, the primary test came back at partial Spearman -0.451, cluster-bootstrap CI [-0.723, -0.041]. The interval excludes zero, and it does so on the side opposite the M1 prediction: **more of the concept direction lying in the output-effective subspace goes with *less* controllability, not more.** This is a stronger result than the null Section 2.5 was written to make publishable. The first-order read/write account is not merely silent about the gap on this data, it is inverted.

The inversion has a clean driver, visible without any statistics. `topic_science` has the highest output overlap of any concept on all four models (0.395 Qwen, 0.424 Mistral, 0.419 Llama, 0.536 Gemma) and simultaneously the lowest controllability on all four (0.019, 0.024, 0.011, 0.007). The concept the output head reads most strongly is the one steering moves least. That is the M1 spectator-feature picture turned exactly on its head.

The most plausible mechanistic reading, offered as a hypothesis for the paper rather than a claim: output overlap as operationalized here (alignment of the concept direction with the top singular subspace of the unembedding) indexes how *lexical* a concept is, not how much the downstream behavior computation is sensitive to it. A concept that maps onto specific output tokens (domain vocabulary for `topic_science`, sheer token count for `verbosity`) projects strongly into the unembedding subspace, yet steering along that direction perturbs token selection into incoherence (it hits the fluency ceiling) or the model re-derives the domain from surviving context, rather than smoothly moving a behavior. Behavioral concepts like `sentiment` have lower output overlap but engage a downstream computation that responds smoothly, so they steer. On that reading the feature is tracking surface-ness, and the surface-confounded concepts flagged by the audit (`topic_science`, `verbosity`) are exactly the high-overlap, immovable ones, which ties the two threads of the paper together.

Two honesty checks belong with this. First, the inversion is not one mechanism for every immovable concept: `refusal` sits at *low* output overlap (0.345 to 0.382) and low controllability, which fits M2 (redundant, multi-direction encoding, as the refusal paper found) rather than the lexical story, so the immovable set is heterogeneous and the paper should say so. Second, the readability axis is saturated (31 of 40 points at AUROC 1.000), so conditioning on readability does little work in the partial correlation, and 40 points across four families is a small sample. Report the sign with its CI, present the lexicality reading as a hypothesis with the `topic_science` inversion as its motivating evidence, and do not oversell a mechanism that a fifth model or a de-saturated readability axis could still move.
...
- **H1 comes back significant in the wrong direction (this is what happened at 40 points):** report it as a directional contradiction of M1, not as a null and not as support. See the dated block in Section 2.5 for the full framing: partial rho -0.451, CI [-0.723, -0.041] excluding zero on the side opposite the prediction, driven by `topic_science` being both the highest-overlap and least-controllable concept on every model. The honest headline is that the first-order read/write account is inverted on this data, with a lexicality reading offered as the mechanistic hypothesis. This is a stronger result than the anticipated null, and the code enforces the distinction (a wrong-signed CI-excluding-zero result cannot be printed as support; see revision R24 below).
...
### Fourth pass: 40-point results, H1 inverted (2026-09-04)

**R24. Section 2.5 reframed for a wrong-signed H1, and the verdict code made sign-aware.** With four models and 40 points the primary test came back significant in the direction opposite the M1 prediction (partial rho -0.451, CI [-0.723, -0.041] excluding zero). Two things were done in response. First, Section 2.5 gains a dated interpretation block: the preregistered M1 prediction is left verbatim, and the observed inversion is framed as a directional contradiction stronger than the null the section was written to survive, with the driver named (`topic_science` is the highest-overlap and least-controllable concept on all four models) and a lexicality reading offered as the mechanistic hypothesis that also unifies with the surface-confound story. The Experiment 4 fallback list gains the wrong-signed branch explicitly. Second, `geometry.primary_test` had a real bug: its wrong-sign guard was gated behind `abs(rho) >= 0.7`, so a moderate wrong-signed result whose CI excludes zero fell through to a sign-blind branch and printed as "suggestive support". The verdict chain is now sign-aware at every magnitude, with two behavioral regression tests (the prior coverage was source-inspection only, which is why the gate slipped through). This is the R10 lesson again in a different place: a plausible-looking figure or verdict string can encode a claim the data does not support, and the check has to be a test, not a comment. Committed to iclr as `4d6ef41`.

**Still open after the fourth pass.** Judge agreement is poor on all four models (0.132 / 0.309 / 0.300 / 0.149) and `human_labels.csv` is unfilled, so objection 14 is unanswered; this is now the single highest-value task and it needs hand labels, not GPU. The readability axis is saturated (31 of 40 points at AUROC 1.000), which weakens both the headline correlation and the partialling in H1. The dissociation is confirmed only on `topic_science`, which is surface-confounded, with `refusal` the clean near-miss sitting on the P6 boundary across all four models. A fifth model family is optional upside; the map itself is complete and should not be re-run.
